	...
	**/
	...
	public function get_menu($identity){
		...
		foreach ($datos as $dato) {
			...
			$array_visible[$dato->menuid] = isset($array_visible[$dato->menuid]) ? $array_visible[$dato->menuid] : 0;
			if(isset($app_array[$dato->menuid])){
				array_push($app_array[$dato->menuid], array(
							'appid' => $dato->appid,
			...
			$array_visible[$dato->menuid] = $dato->visible == 1 ? $array_visible[$dato->menuid] + 1 : $array_visible[$dato->menuid];
		}


		foreach ($datos as $dato) {
			if(!isset($menu_array[$dato->menuid])){
				$menu_array[$dato->menuid] = array();
				array_push($menu_list,array(
						'menuid' => $dato->menuid,
						'menuname' => $dato->menuname,
	...
	public function update_level($userid,$level){
		$this->db->where('id', $userid);		
		$this->db->update($this->tables['users'],array('level' => $level)); 					
	}
	...
	/*public function asocia_comunidad($userid,$idcomunidad){
		...
		if(count($datos) > 0){ // si ya existe, actualiza

			$data = array(
						'idcomunidad' => $idcomunidad
					);
			$this->db->where('idusuario', $userid);
			$this->db->update('gc_usuario_comunidad',$data);
		}else{
			...
			$this->db->insert('gc_usuario_comunidad',$data);
		}
	}*/
	...
	/*public function asocia_propiedad($userid,$idpropiedad){
		$this->db->select(' idusuario ')
						  ->from('gc_usuario_propiedad')
		                  ->where('idusuario', $userid);
		$query = $this->db->get();
		$datos = $query->result();
		...
		if(count($datos) > 0){ // si ya existe, actualiza
			...
			$this->db->where('idusuario', $userid);
			$this->db->update('gc_usuario_propiedad',$data);
			...
			$this->db->insert('gc_usuario_propiedad',$data);
		}

	}*/	

	public function asocia_propiedad($userid,$array_propiedades,$replace = true){
	...
	public function update_comunidad($userid,$idcomunidad){
		$data = array(
					'idcomunidad' => $idcomunidad
				);
		$this->db->where('idusuario', $userid);
	...
	public function update_propiedad($userid,$idpropiedad){
	...
	/*public function log($action,$detail = ''){
			...
			$data = array(
						...
						'action' => $action,
						'detail' => $detail,
						'ipaddress' => $this->_prepare_ip($this->input->ip_address())
					);

			$this->db->insert('gc_access_log',$data);	
